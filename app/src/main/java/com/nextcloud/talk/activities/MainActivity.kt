...
class MainActivity : BaseActivity(), ActionBarProvider {
    ...
    /**
     * Checks for saved credentials in AccountManager and redirects to ConversationsListActivity
     * if valid credentials are found
     */
    private fun checkForSavedCredentials() {
        try {
            val accountManager = AccountManager.get(this)
            val accountType = "com.nextcloud.talk"
            val accounts = accountManager.getAccountsByType(accountType)
            
            if (accounts.isNotEmpty()) {
                // If we have saved accounts, get the most recently used one
                val account = accounts[0]
                val username = account.name.substringBefore('@')
                val server = "https://" + account.name.substringAfter('@')
                val token = accountManager.getPassword(account)
                
                if (!username.isNullOrEmpty() && !token.isNullOrEmpty()) {
                    Log.d(TAG, "Found saved credentials, navigating to conversations")
                    
                    // Get the user from database
                    userManager.checkIfUserExists(username, server)
                        .subscribe { exists ->
                            if (exists) {
                                // User exists in database, navigate to conversations
            ...
            // Continue with normal app flow if credentials retrieval failed
        }
    }
