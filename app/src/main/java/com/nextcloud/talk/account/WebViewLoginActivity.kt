...
class WebViewLoginActivity : BaseActivity() {
    ...
    private fun parseAndLoginFromWebView(dataString: String) {
        ...
        if (loginData != null) {
            ...
            // Store username and password in AccountManager for future use
            saveAccount(loginData.username!!, loginData.token!!, baseUrl!!)
    ...
    /**
     * Stores user credentials securely using Android's AccountManager
     */
    private fun saveAccount(username: String, token: String, serverUrl: String) {
        val account = Account(username + "@" + serverUrl.replace("https://", "").replace("http://", ""), "com.nextcloud.talk")
        val accountManager = AccountManager.get(this)
        
        // Store the raw username which is needed for authentication
        accountManager.addAccountExplicitly(account, token, null)
        accountManager.setUserData(account, "server_url", serverUrl)
        accountManager.setUserData(account, "raw_username", username)
